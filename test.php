...
function displayTestingConfirmation($uid, $pid) {
    ...
    $currentlyTesting = getPuzzlesInTestQueue($uid);

    if (!empty($currentlyTesting)) {
?>
    <div class="errormsg">
        <p>You haven't filled out a testsolve report for puzzle(s):</p>
        <ul>
<?php
        foreach ($currentlyTesting as $testingPid) {
?>
            <li><a href="<?php echo URL . "/test.php?pid=" . $testingPid; ?>"><?php echo getTitle($testingPid); ?></a></li>
<?php
        }
?>
        </ul>

      <p><strong>Before</strong> starting any new test solves, please submit a testing report for the puzzles you are currently testing.</p>
    </div>
<?php
    }
?>
    <div class="testwarning">
        ...
        <ul>
            ...
            <li>If you do click it, you are expected to fill out a testsolving report <strong>within 48 hours</strong>.  (It's ok if you haven't finished the puzzle by then, and just report your progress so far, or if you decide not to solve it after all and say so in the report.)  Please only click if you intend to fill out a testsolving report promptly!!</li>
        </ul>
